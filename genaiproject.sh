...
filename="buffer.txt"
touch "$filename"
user_prompt="Give me ${no} questions on the topic ${topic} at an undergrad/college level and of ${difficulty} 
...
generate_questions | pv -i 0.1 | zenity --progress --text="Creating Questions..." --auto-close --pulsate
wait
# cat "$filename"

# wait $(pgrep zenity)
choice=$(zenity --question --title "Quiz creation" --text="$return_filename created. Do you want to convert the file to a .docx format?" --ok-label="Yes" --cancel-label="No")

if [ $? -eq 0 ]; then
  pandoc -o "${return_filename%.*}.docx" "$return_filename"
  rm "$return_filename"
  zenity --info --text "${return_filename%.*}.docx created." | wait 1
fi
